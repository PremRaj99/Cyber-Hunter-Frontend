    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Modal */}
          <motion.div
            className="relative w-full max-w-md p-6 mx-4 bg-gray-900 border border-brandPrimary/30 rounded-lg shadow-xl shadow-brandPrimary/10 overflow-hidden"
            role="dialog"
            aria-modal="true"
          ...
          >
            {/* Cyan accent border */}
            <motion.div
              className="absolute inset-x-0 top-0 h-1 bg-brandPrimary"
              initial={{ scaleX: 0 }}
              animate={{ scaleX: 1 }}
              exit={{ scaleX: 0 }}
            ...
            <div className="flex flex-col items-center sm:items-start">
              ...
              <div className="w-full flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                <motion.button
                  type="button"
                  className="w-full sm:w-auto px-4 py-2 bg-gray-800 border border-gray-700 rounded-md text-white font-medium hover:bg-gray-700 focus:outline-none focus:ring-brandPrimary transition-colors duration-200"
                  onClick={onClose}
